...
widget.line = {
	...
	settingCreated: function($content, settings){// setting == undefined mean is create not modify
			...
			$("<table></table>").attr({
				...
				$("<tr></tr>").append(
					$("<td></td>").attr({
						"colSpan": "999",
						"class": "content-title"
					}).css("paddingBottom", "10px").text("<?=$lang['WIDGET']['LINE']['DRAWING'][$language]?>")
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css("padding", "3px 50px 3px 0").text("<?=$lang['WIDGET']['LINE']['FILL_OPACITY'][$language]?>")
				).append(
				...
				$("<tr></tr>").append(
					$("<td></td>").css("padding", "3px 50px 3px 0").text("<?=$lang['WIDGET']['LINE']['FILL_GRADIENT'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<select></select>").attr("id", "widget-draw-fill-gradient").css("width", "100%").append(
							$("<option></option>").val("1").text("<?=$lang['WIDGET']['LINE']['ENABLE'][$language]?>").attr("selected", true)
						).append(
							$("<option></option>").val("0").text("<?=$lang['WIDGET']['LINE']['DISABLE'][$language]?>")
						)
					)
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css("padding", "3px 50px 3px 0").text("<?=$lang['WIDGET']['LINE']['LINE_WIDTH'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
				...
				$("<tr></tr>").append(
					$("<td></td>").css("padding", "3px 50px 3px 0").text("<?=$lang['WIDGET']['LINE']['POINT'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<select></select>").attr("id", "widget-draw-points").css("width", "100%").append(
							$("<option></option>").val("1").text("<?=$lang['WIDGET']['LINE']['SHOW'][$language]?>").attr("selected", true)

						).append(
							$("<option></option>").val("0").text("<?=$lang['WIDGET']['LINE']['HIDE'][$language]?>")
						)
					)
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css("padding", "3px 50px 3px 0").text("<?=$lang['WIDGET']['LINE']['STACKING'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<select></select>").attr("id", "widget-draw-stack").css("width", "100%").append(
							$("<option></option>").val("1").text("<?=$lang['WIDGET']['LINE']['ENABLE'][$language]?>")
						).append(
							$("<option></option>").val("0").text("<?=$lang['WIDGET']['LINE']['DISABLE'][$language]?>").attr("selected", true)
						)
					)
				)
			).append(
				...
				$("<tr></tr>").append(
					$("<td></td>").attr({
						"colSpan": "999",
						"class": "content-title"
					}).css("paddingBottom", "10px").text("<?=$lang['WIDGET']['LINE']['LEGEND'][$language]?>")
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css({
						"padding": "3px 50px 3px 0"
					}).text("<?=$lang['WIDGET']['LINE']['LOCATION'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<select></select>").attr("id", "widget-legend-show").css("width", "100%").append(
							$("<option></option>").val("2").text("<?=$lang['WIDGET']['LINE']['RIGHT'][$language]?>")
						).append(
							$("<option></option>").val("1").text("<?=$lang['WIDGET']['LINE']['BOTTOM'][$language]?>")
						).append(
							$("<option></option>").val("0").text("<?=$lang['WIDGET']['LINE']['DISABLE'][$language]?>").attr("selected", true)
						)
					)
				...
				$("<tr></tr>").append(
					$("<td></td>").attr({
						"colSpan": "999",
						"class": "content-title"
					}).css("paddingBottom", "10px").text("<?=$lang['WIDGET']['LINE']['Y_AXIS'][$language]?>")
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css({
					...
					$("<td></td>").css("padding", "3px 0").append(
						$("<select></select>").attr("id", "widget-yaxis-position").css("width", "100%").append(
							$("<option></option>").val("right").text("<?=$lang['WIDGET']['LINE']['RIGHT'][$language]?>").attr("selected", true)
						).append(
							$("<option></option>").val("left").text("<?=$lang['WIDGET']['LINE']['LEFT'][$language]?>")
						)
					)
				)
			).append(
				$("<tr></tr>").append(
					$("<td></td>").css({
						"padding": "3px 50px 3px 0"
					}).text("<?=$lang['WIDGET']['LINE']['MAXIMUM'][$language]?>")// 
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<input/>").attr({
							"id": "widget-yaxis-max",
							"type": "text",
							"placeholder": "<?=$lang['WIDGET']['LINE']['AUTO'][$language]?>"
						}).css({
							"width": "100%",
				...
				$("<tr></tr>").append(
					$("<td></td>").css({
						"padding": "3px 50px 3px 0"
					}).text("<?=$lang['WIDGET']['LINE']['MINIMUM'][$language]?>")
				).append(
					$("<td></td>").css("padding", "3px 0").append(
						$("<input/>").attr({
							"id": "widget-yaxis-min",
		...
		if(settings){
			...
			$("#widget-draw-points").val(settings.draw.points == true ? "1" : "0");
	...
	settingSaved: function(settings, channels){
		settings.draw = {
			...
			"points": $("#widget-draw-points").val() == "1" ? true : false,
	...
	drawChart: function($content, settings, channels){
		...
		var option = {
			...
			tooltipOpts: {
				content: function(label, xval, yval, flotItem){
					...
					var table = "<table>";
					table += "<tr><td align='right' style='font-weight:bold;'><?=$lang['WIDGET']['LINE']['TIP']['CHANNEL'][$language]?></td><td>" + flotItem.series.tipText + "</td></tr>";
					table += "<tr><td align='right' style='font-weight:bold;'><?=$lang['WIDGET']['LINE']['TIP']['TIME'][$language]?></td><td>" + date.getFullYear() + "/" + padding(date.getMonth() + 1, 2) + "/" + padding(date.getDate(), 2) + " " + padding(date.getHours(), 2) + ":" + padding(date.getMinutes(), 2) + ":" + padding(date.getSeconds(), 2) + "</td></tr>";
					table += "<tr><td align='right' style='font-weight:bold;'><?=$lang['WIDGET']['LINE']['TIP']['VALUE'][$language]?></td><td>" + numberWithCommas(yval) + "&nbsp;" + flotItem.series.unit + "</td></tr>";
					table += "</table>";
					return table;
				}
			}
		...
		if(isDark){ // black
			...
			option.series["points"]["fillColor"] = "#252525";
		}
