...
//            }
        ...
        Gauge.prototype.drawValue = function(gaugeOptionsi, layout, cellLayout, i, item) {
			...
			if (!exists) {
				var $value = $(placeholder).find("#flotGaugeValue" + i).css("fontWeight", "bold");

				if(typeof(gaugeOptionsi.value.tooltip) == "function"){
					$value.bind("mousemove", function(event){
						...
						if($tip.length == 0){
							...
							$tip = $("body > div.flotTip").clone().removeClass("flotTip").addClass("valueTip").attr("id", id).html(gaugeOptionsi.value.tooltip(item.label, item.data[0][1], item)).show();
        ...
        plot.hooks.draw.push(function(plot, context) {
            ...
            for (var i = 0; i < series.length; i++) {
                ...
                if (item.gauges) {
					...
					gaugeOptionsi.value.font.size = layout.radius * 0.4;
                }
